...
const TOKEN_FILE  = path.join(__dirname, "../.admin-token.json");

/** Read cached admin token or acquire a fresh one (writes cache). */
async function getAdminTokenCached(page: Page): Promise<string> {
  // Try reading from cache first
  try {
    const cached = JSON.parse(fs.readFileSync(TOKEN_FILE, "utf8"));
    if (cached.token && cached.expires > Date.now()) return cached.token as string;
  } catch { /* no cache */ }
...
async function waitForPhase(page: Page, phase: "betting" | "flying" | "crashed") {
  await page.locator(`[data-phase="${phase}"]`).first().waitFor({ state: "visible", timeout: 40000 });
}

/** Wait for a FRESH betting round with at least `minMs` ms left in the countdown. */
async function waitForFreshBetting(page: Page, minMs = 3500) {
...
async function getAdminToken(page: Page): Promise<string> {
  return getAdminTokenCached(page);
}
...
test.describe("2. Authentication", () => {
  ...
  test("2.5 UI: admin user sees admin panel, not game", async ({ page }) => {
    ...
    // Admin panel should be visible via data-testid
...
test.describe("4. Full game flow", () => {
  ...
  test("4.3 Placing a bet deducts balance", async ({ page }) => {
    ...
    // Wait for a fresh betting phase with enough countdown
    await waitForFreshBetting(page);
  ...
  test("4.5 Cashing out increases balance", async ({ page }) => {
    ...
    await waitForFreshBetting(page, 2000);
